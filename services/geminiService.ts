...
export const generateAiPersona = async (
    ...
    CRITICAL: You MUST also generate 2-3 "Jobs To Be Done" (JTBD) entries.
    ...
    const textResponse = await callAi(prompt, schema);
    const parsedData = parseJsonFromText<Partial<Persona>>(textResponse || "{}");

    if (parsedData && parsedData.jobsToBeDone) {
        // Add IDs to jobsToBeDone as they are required by the UI but not generated by AI (or to ensure uniqueness)
        parsedData.jobsToBeDone = parsedData.jobsToBeDone.map((job: any) => ({
            ...job,
            id: `jtbd-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
        }));
    }
