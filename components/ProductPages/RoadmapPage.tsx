  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 p-6 md:p-12">
      <div className="max-w-5xl mx-auto">
          ...
          <div className="space-y-12">
            {roadmapData.map((phase, index) => {
              ...
              return (
                ...
                >
                  {/* Timeline Dot */}
                  <div className="absolute left-4 md:left-1/2 w-4 h-4 rounded-full border-4 border-slate-900 z-10 -translate-x-1/2 mt-6
                    ${phase.status === 'Completed' ? 'bg-emerald-500' : phase.current ? 'bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.5)]' : 'bg-slate-600'} hidden md:block"
                  ></div>

                  {/* Content Card */}
                  <div className="flex-1"></div> {/* Spacer */}
                  <div className="flex-1">
                    <div className={`bg-slate-800 p-6 rounded-2xl border ${phase.current ? 'border-blue-500/50 shadow-lg shadow-blue-500/10' : 'border-slate-700'} relative`}>
                      {phase.current && (
        ...
        <div className="mt-20 text-center bg-slate-800/50 rounded-2xl p-8 border border-slate-700">
